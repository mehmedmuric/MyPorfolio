...
const SingleFeature = ({ feature, index }: { feature: Feature; index: number }) => {
  ...
  return (
    ...
    >
      <div className="wow fadeInUp transition-all duration-500" data-wow-delay={`${index * 0.1}s`} data-animate="fade-in-up">
        ...
        <div className="relative mb-6 group/icon">
          ...
          <div className="relative flex h-16 w-16 sm:h-20 sm:w-20 md:h-20 md:w-20 items-center justify-center rounded-xl bg-gradient-to-br from-green-500/10 to-green-500/5 border border-green-500/30 group-hover/icon:border-green-500/60 group-hover/icon:scale-110 transition-all duration-500 shadow-[0_0_20px_rgba(0,255,128,0.2)] group-hover/icon:shadow-[0_0_40px_rgba(0,255,128,0.4)]">
            <div className={`transform transition-transform duration-500 ${isHovered ? 'scale-110 rotate-6' : 'scale-100 rotate-0'}`}>
              {icon}
            </div>
            {/* Number badge */}
            <div className={`absolute -top-2 -right-2 flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full bg-green-500 text-black text-xs font-bold shadow-lg transition-all duration-300 ${isHovered ? 'scale-110 shadow-[0_0_15px_rgba(0,255,128,0.6)]' : 'scale-100'}`}>
        ...
        <h3 className="mb-4 text-lg sm:text-xl md:text-xl lg:text-xl xl:text-2xl font-bold text-white tracking-wide group-hover:text-green-400 transition-colors duration-300">
